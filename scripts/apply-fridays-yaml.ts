/**
 * Aplica cascata de sextas ao sessions.yaml (seed).
 * Uso: npx tsx scripts/apply-fridays-yaml.ts
 */
import fs from 'node:fs'
import path from 'node:path'
import { fileURLToPath } from 'node:url'
import { dump, load } from 'js-yaml'
import { computeFridayFixChanges } from '../shared/fridayMigration.ts'
...
function main() {
  ...
  if (changes.length === 0) {
    console.log('Nenhuma sessao agendada em sexta no YAML.')
    return
  }
